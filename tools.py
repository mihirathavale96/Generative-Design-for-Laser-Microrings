...
def save_sample_image(images: torch.Tensor, to_grayscale: bool = False, **kwargs):
    """
    ...
    for i in range(images.shape[0]):
        ...
        t = make_grid(images[i], nrow=images.shape[1])  # (channels, height, width)
        grid.append(t)

    # stack all merged images to a tensor
    grid = torch.stack(grid, dim=0)  # (batch_size, channels, height, width)
    grid = make_grid(grid, nrow=1)  # concat all batch images in a different row, (channels, height, width)

    # Convert to (height, width, channels)
    grid = grid.permute(1, 2, 0).to("cpu", torch.uint8).numpy()

    im = Image.fromarray(grid, mode='L') # Force grayscale mode
    im.save(path, format=format)